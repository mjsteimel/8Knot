...
PAGE = "starter_health"
VIZ_ID = "change-request-close-ratio"

gc_change_req_close_ratio = dbc.Card(
    [
        dbc.CardBody(
            [
                html.H3(
                ...
                dbc.Popover(
                    [
                        ...
                        dbc.PopoverBody(
                            """
                            Visualizes the ratio of the total number of open change requests to the total number of closed change requests over time.
                            """
                        ),
                    ],
...
# callback for change request close ratio over time graph
@callback(
...
def change_request_close_ratio_graph(repolist, interval):
    ...
    logging.warning("CHANGE REQUEST CLOSURE RATIO - START")

    # test if there is data
    if df.empty:
        logging.warning("CHANGE REQUEST CLOSURE RATIO - NO DATA AVAILABLE")
        return nodata_graph

    # function for all data pre processing
    df_created, df_closed_merged, df_open = process_data(df, interval)

    fig = create_change_request_close_ratio_figure(df_created, df_closed_merged, df_open, interval)

    logging.warning(f"CHANGE_REQUEST_CLOSURE_RATIO_VIZ - END - {time.perf_counter() - start}")

    return fig

def create_change_request_close_ratio_figure(
    ...
    # calculate the ratio of open to closed change requests for each year
    df_ratio = pd.DataFrame()
    df_ratio["Date"] = df_created["Date"]
    df_ratio["Ratio"] = df_open["Open"] / (df_closed_merged["merged"] + df_closed_merged["closed"])
    ...
    fig.add_bar(
        ...
        y=df_ratio["Ratio"],
        opacity=0.9,
        hovertemplate="Ratio: %{y:.2f}<br>%{x|%b %d, %Y} <extra></extra>",
        marker=dict(color=color_seq[0]),
        name="Change Request Close Ratio",
    )
    fig.update_xaxes(
        showgrid=True,
...
def process_data(df: pd.DataFrame, interval):
    ...
    # ----- Open PR processing starts here ----
