...
PAGE = "starter"
VIZ_ID = "issues-closed"

gc_issues_closed = dbc.Card(
    [
        dbc.CardBody(
            [
                html.H3(
                ...
                dbc.Popover(
                    [
                        ...
                        dbc.PopoverBody(
                            """
                            Visualizes the activity of issues being Closed partitioned by time-window.\n
...
def issues_closed_graph(repolist, interval):
...
def process_data(df: pd.DataFrame, interval):
    ...
    # df for closed issues in the time interval
    ...
    # df for open issues for the time interval
    df_open = dates.to_frame(index=False, name="Date")

    # applies function to get the amount of open issues for each day
...
def get_open(df, date):
    ...
    # include issues that have not been closed yet
    df_open = pd.concat([df_open, df_lim[df_lim.closed.isnull()]])

    # generates the number of columns, i.e., open issues
